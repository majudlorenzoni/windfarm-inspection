...
function CameraController() {
  ...
  useFrame(() => {
    ...
    if (moveState.current.forward)
      camera.position.addScaledVector(direction, CAMERA_SPEED);
    if (moveState.current.backward)
      camera.position.addScaledVector(direction, -CAMERA_SPEED);
    if (moveState.current.left)
      camera.position.addScaledVector(sideDirection, CAMERA_SPEED);
    ...
    camera.position.x = Math.min(
      Math.max(camera.position.x, -halfPlane),
      halfPlane,
    );
    camera.position.z = Math.min(
      Math.max(camera.position.z, -halfPlane),
      halfPlane,
    );
