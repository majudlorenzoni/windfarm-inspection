...
export const RelatorioGeral: React.FC = () => {
      ...
      if (savedData) {
        try {
          ...
          if (parsedData.createdAt && parsedData.createdAt.seconds) {
            parsedData.createdAt = new Date(parsedData.createdAt.seconds * 1000);
          }
  ...
  useEffect(() => {
    if (windData) {
      const toSave = {
        ...windData,
        createdAt: windData.createdAt instanceof Date 
  ...
  if (!windData) {
    return <RelatorioGeralContainer><p>Nenhuma inspeção carregada.</p></RelatorioGeralContainer>;
  }
  ...
  const inspecionadas = turbines.filter((t: any) => t.maintenance_log && t.maintenance_log.length > 0).length;
  ...
  return (
    <RelatorioGeralContainer>
      ...
      <SectionContainer>
        <SectionHeader onClick={() => toggleSection('sumario')}>Sumário</SectionHeader>
          <SectionContent>
            <p>Esta inspeção foi realizada em {windData?.createdAt?.toDate?.().toLocaleString?.() || 'data desconhecida'}.</p>
            <p>Foram avaliadas {total} turbinas. Abaixo, seguem os gráficos e dados técnicos da operação.</p>
          </SectionContent>
      </SectionContainer>
    ...
    <ChartGrid>
              ...
              <PieChart>
                <Pie
                  data={[
                    ...
                    { name: 'Sem falha conhecida', value: inspecionadas - falhas },
                  ]}
                  cx="50%"
                  cy="50%"
                  outerRadius={100}
      ...
      <SectionContainer>
        <SectionHeader onClick={() => toggleSection('graficoUptime')}>Gráfico: Disponibilidade (%)</SectionHeader>
          <SectionContent>
            <ResponsiveContainer width="100%" height={340}>
              <LineChart
              ...
              >
                ...
                <Tooltip />
                <Legend />
                <Line type="monotone" dataKey="uptime" stroke="#a78bfa" name="Disponibilidade" />
              </LineChart>
            </ResponsiveContainer>
          </SectionContent>
      </SectionContainer>
          ...
          <SectionContent>
            <ResponsiveContainer width="100%" height={300}>
              ...
              >
                ...
                <Bar dataKey="amplitude" fill="#f87171" name="Amplitude de Vibração (g)" />
              </BarChart>
            </ResponsiveContainer>
          </SectionContent>
        </SectionContainer>
          ...
          <SectionContent>
            <ResponsiveContainer width="100%" height={300}>
              ...
              >
                ...
                <YAxis
                  stroke="#000"
                  label={{ value: 'MPa', angle: -90, position: 'insideLeft', fill: '#000' }}
                />
                <Tooltip contentStyle={{ backgroundColor: "#fff", color: "#000" }} />
                <Legend wrapperStyle={{ color: "#000" }} />
